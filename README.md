...
- **Companion channel (experimental, opt-in)**: when every backend path is shut, the integration can read your car by driving the official app on a spare Android phone. This fork includes a small [Android AccessibilityService agent](android/companion-agent/) that opens an authenticated outbound HTTPS relay to Home Assistant; ADB is needed only to install/update it, so Wireless Debugging can stay off during normal operation. Nothing is rooted and no app tokens are read.
